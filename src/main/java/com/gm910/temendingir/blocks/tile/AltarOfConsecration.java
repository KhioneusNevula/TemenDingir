...
public class AltarOfConsecration extends TileEntity implements ITickableTileEntity {
	...
	private static BlockPattern getGenerationPattern() {
		if (altarOfConsecrationPattern == null) {
			altarOfConsecrationPattern = BlockPatternBuilder.start().aisle("???", "?t?", "???").aisle("sss", "sls", "sss")
	...
	public static void createFireOfCreation(BlockEvent.EntityPlaceEvent event) {
		...
		if (event.getPlacedBlock().getBlock() == Blocks.TORCH) {
			...
			((AltarOfConsecration) world.getTileEntity(origin.up())).invoked = d;
			for (BlockPos posf : framePositions) {
				if (d != null) {
