...
public class TaskChangeBlock extends ModTask {
	...
	public void run() {
		...
		TileEntity te = tile != null ? TileEntity.readTileEntity(state, tile): null;
		if (world != null) {
			if (world.isRemote) {
				if (world.getDimensionKey().getLocation().equals(this.pos.getD())) {
				...
				world = world.getServer().getWorld(world.getDimensionKey());
				world.setBlockState(pos, state, flag);
				if (te != null) {
					world.setTileEntity(pos, te);
				}
