...
public class MagicLightning extends LightningBoltEntity {
	...
	public MagicLightning(EntityType<? extends MagicLightning> en, World worldIn) {
		super(en, worldIn);
		this.ignoreFrustumCheck = true;
		this.lightningState = 2;
		this.boltVertex = this.rand.nextLong();
		this.boltLivingTime = this.rand.nextInt(3) + 1;
		this.source = DamageSource.LIGHTNING_BOLT;

	}

	public MagicLightning(World world, DamageSource source) {
		this(EntityInit.MAGIC_LIGHTNING.get(), world);
		this.source = source;
	}

	public void setDoFire(boolean doFire) {
		this.doFire = doFire;
	}
	...
	public void tick() {
		...
		if (this.lightningState == 2) {
			...
			this.world.playSound((PlayerEntity) null, this.getPosX(), this.getPosY(), this.getPosZ(),
					SoundEvents.ENTITY_LIGHTNING_BOLT_THUNDER, SoundCategory.WEATHER, 10000.0F,
					0.8F + this.rand.nextFloat() * 0.2F);
			this.world.playSound((PlayerEntity) null, this.getPosX(), this.getPosY(), this.getPosZ(),
					SoundEvents.BLOCK_BELL_RESONATE, SoundCategory.WEATHER, 2.0F, 0.5F + this.rand.nextFloat() * 0.2F);
		}
		...
		if (this.lightningState >= 0) {
			if (!(this.world instanceof ServerWorld)) {
				...
				List<Entity> list = this.world.getEntitiesInAABBexcluding(this,
						new AxisAlignedBB(this.getPosX() - 3.0D, this.getPosY() - 3.0D, this.getPosZ() - 3.0D,
								this.getPosX() + 3.0D, this.getPosY() + 6.0D + 3.0D, this.getPosZ() + 3.0D),
						Entity::isAlive);

				for (Entity entity : list) {
					if (!net.minecraftforge.event.ForgeEventFactory.onEntityStruckByLightning(entity, this)) {

						this.attackEntityFrom(source, 5.0F);
					}
				}
	...
	private void igniteBlocks(int extraIgnitions) {
		if (!this.effectOnly && this.doesFire() && !this.world.isRemote
